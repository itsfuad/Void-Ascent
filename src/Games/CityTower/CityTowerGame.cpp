...
static uint16_t blend565(uint16_t first, uint16_t second, uint8_t amount) {
  ...
  const uint16_t red = (firstRed * (255 - amount) + secondRed * amount) >> 8;
  const uint16_t green =
      (firstGreen * (255 - amount) + secondGreen * amount) >> 8;
  const uint16_t blue = (firstBlue * (255 - amount) + secondBlue * amount) >> 8;
...
static float currentAnchorX(uint32_t now) {
  // The trolley stays fixed. Only the rope and block swing.
  (void)now;
...
static void spawnParticle(float x, float y, float velocityX, float velocityY,
...
static void spawnLandingBurst(float x, float y, bool perfect) {
  ...
  for (uint8_t index = 0; index < count; ++index) {
    ...
    const uint16_t colour = perfect ? (index & 1 ? C_GOLD : C_GREEN) : C_STONE;
...
static void spawnBlock(uint32_t now) {
  ...
  block.swingSpeed = clampFloat(1.30f + city.floorCount * 0.026f, 1.30f, 2.10f);

  block.swingAmplitude =
      clampFloat(0.18f + city.floorCount * 0.0045f, 0.18f, 0.34f);

  block.ropeLength = clampFloat(88.0f - city.floorCount * 0.08f, 84.0f, 88.0f);
...
static void releaseBlock() {
  ...
  // The release must always begin downward. Without this clamp, tangential
  // pendulum velocity could briefly launch the block upward.
...
static bool centreOfMassHasSupport(float centreOfMass, float lowerCentre,
  ...
  const float margin = STABILITY_MARGIN < contactWidth * 0.18f
                           ? STABILITY_MARGIN
                           : contactWidth * 0.18f;
...
// Returns the first floor in the unsupported upper section. A negative result
// means that every contact can support the combined mass above it.
static int8_t firstUnstableFloor() {
  ...
  if (!centreOfMassHasSupport(wholeTowerCentre, SCREEN_W * 0.5f,
                              FOUNDATION_WIDTH, city.floors[0].centreX,
                              city.floors[0].width)) {
    return 0;
  }

  for (uint8_t lowerIndex = 0; lowerIndex + 1 < city.floorCount; ++lowerIndex) {
    float upperSum = 0.0f;
    uint8_t upperCount = 0;

    for (uint8_t upperIndex = lowerIndex + 1; upperIndex < city.floorCount;
         ++upperIndex) {
...
static float towerCrookedness() {
  ...
  const float centreOffset = fabsf(towerCentreOfMass() - SCREEN_W * 0.5f);

  const float stepStress = maximumStep / (FLOOR_WIDTH * 0.34f);

  const float centreStress = centreOffset / (FOUNDATION_WIDTH * 0.18f);

  return clampFloat(stepStress > centreStress ? stepStress : centreStress, 0.0f,
                    1.6f);
}
...
static void updateTowerMotion(float deltaSeconds, uint32_t now) {
  ...
  // Crooked towers have a biased resting angle. Taller structures have a
  // softer spring and continue moving for longer after an impact.
  ...
  // Straight short towers barely move. Height and crookedness amplify wind.
  const float wind = sinf(now * 0.00108f + 0.65f) *
                     (0.0020f + floorFactor * 0.0032f) *
                     (0.40f + crookedness * 0.75f);
...
// Oscillation can push a statically valid, but badly crooked, tower beyond a
// contact edge. This check uses the instantaneous dynamic tower angle.
static int8_t firstDynamicallyUnstableFloor() {
  ...
  if (!centreOfMassHasSupport(wholeCentre, SCREEN_W * 0.5f, FOUNDATION_WIDTH,
  ...
  for (uint8_t lowerIndex = 0; lowerIndex + 1 < city.floorCount; ++lowerIndex) {
    ...
    for (uint8_t upperIndex = lowerIndex + 1; upperIndex < city.floorCount;
      ...
      upperRelativeHeight += (upperIndex + 0.5f) * FLOOR_HEIGHT - contactHeight;
...
static void beginTowerCollapse(uint8_t startIndex, uint32_t now) {
  ...
  for (uint8_t index = startIndex; index < city.floorCount; ++index) {
    ...
    floor.velocityX = direction * (24.0f + heightAmount * 5.0f) +
                      randomGenerator.range(-5.0f, 5.0f);

    floor.velocityY = -18.0f - heightAmount * 2.5f;
  }

  const float seamY = startIndex == 0
...
static void updateTowerCollapse(float deltaSeconds, uint32_t now) {
  ...
  city.towerLean =
      clampFloat((towerCentreOfMass() - SCREEN_W * 0.5f) * 0.16f, -5.0f, 5.0f);
...
static void missBlock(uint32_t now) {
  ...
  city.active.velocityY =
      city.active.velocityY < 45.0f ? 45.0f : city.active.velocityY;

  setFeedback("MISSED!", C_RED, 1.0f);

  for (uint8_t index = 0; index < 8; ++index) {
    spawnParticle(city.active.centreX, city.active.topY + FLOOR_HEIGHT * 0.5f,
...
static void landBlock(uint32_t now) {
  ...
  const float supportRestCentre =
      previousFloorCount == 0 ? SCREEN_W * 0.5f
  ...
  const float supportWidth = previousFloorCount == 0
                                 ? FOUNDATION_WIDTH
  ...
  // Save the unswung structural coordinate. Removing the current visible
  // offset prevents the floor from snapping after it joins the tower.
  ...
  // The landing offset and horizontal block speed inject rotational momentum.
...
static void updatePlaying(float deltaSeconds, uint32_t now) {
  ...
  if (block.mode == BlockMode::HANGING) {
    ...
    if (block.mode == BlockMode::MISSED &&
        block.topY > SCREEN_H + FLOOR_HEIGHT + 12.0f) {
...
static void drawSky(uint32_t now, bool gameScene) {
  const float nightAmount =
      gameScene ? clampFloat((city.floorCount - 12.0f) / 18.0f, 0.0f, 1.0f)
                : 0.0f;
...
static void drawSkylineLayer(int16_t baseY, uint8_t layer, uint16_t colour,
  ...
  while (x < SCREEN_W) {
    ...
    for (int16_t windowY = y + 7; windowY < baseY - 5; windowY += 12) {
      for (int16_t windowX = x + 4; windowX < x + width - 3; windowX += 8) {
        if (((windowX + windowY + index + now / 1200U) % 5U) == 0) {
          frame.fillRect(windowX, windowY, 2, 3, windowColour);
        }
      }
    }
...
static void drawCityBackground(uint32_t now, bool gameScene) {
  ...
  const int16_t cameraShift =
      gameScene ? (int16_t)(city.cameraWorld * 0.10f) : 0;
...
static void drawFloorBlock(float centreX, float topY, float width,
  ...
  const int16_t blockWidth = (int16_t)roundf(width);

  const int16_t blockHeight = (int16_t)roundf(height);

  if (blockWidth < 12 || blockHeight < 12 || y > SCREEN_H ||
      y + blockHeight < 0 || x > SCREEN_W || x + blockWidth < 0) {
    return;
  }
  ...
  frame.fillRect(x + 3, y + 4, blockWidth, blockHeight, C_INK);

  frame.fillRect(x, y, blockWidth, blockHeight, dark);

  frame.fillRect(x + 3, y + 3, blockWidth - 7, blockHeight - 6, body);

  frame.fillRect(x + 1, y, blockWidth - 2, 3, trim);

  frame.fillRect(x + 2, y + blockHeight - 4, blockWidth - 5, 3, trim);

  frame.drawRect(x, y, blockWidth, blockHeight, C_INK);

  frame.drawFastVLine(x + blockWidth - 4, y + 3, blockHeight - 6,
  ...
  if (blockHeight >= 34) {
    ...
    const int16_t windowWidth = 8;
    const int16_t windowHeight = 9;

    const int16_t usableWidth = blockWidth - 10;

    const int16_t columnStep = usableWidth / WINDOW_COLUMNS;

    const int16_t firstY = y + 7;

    const int16_t rowStep = (blockHeight - 15) / WINDOW_ROWS;

    for (uint8_t row = 0; row < WINDOW_ROWS; ++row) {
      for (uint8_t column = 0; column < WINDOW_COLUMNS; ++column) {
        const int16_t windowX =
            x + 5 + column * columnStep + (columnStep - windowWidth) / 2;

        const int16_t windowY = firstY + row * rowStep;

        const uint8_t bitIndex = (row * WINDOW_COLUMNS + column) * 3;

        const bool lit = ((seed >> bitIndex) & 3U) != 0;

        frame.fillRect(windowX, windowY, windowWidth, windowHeight, C_INK);

        frame.fillRect(windowX + 1, windowY + 1, windowWidth - 2,
                       windowHeight - 2,
                       lit ? C_GOLD : blend565(C_GLASS_DARK, C_SKY, 80));

        frame.drawFastVLine(windowX + windowWidth / 2, windowY + 1,
                            windowHeight - 2, dark);

        frame.drawFastHLine(windowX + 1, windowY + windowHeight / 2,
                            windowWidth - 2, dark);
      }
    }

    frame.drawFastHLine(x + 3, y + blockHeight / 2, blockWidth - 7,
                        blend565(trim, body, 95));

    frame.fillRect(x + blockWidth / 2 - 4, y + blockHeight - 11, 8, 7, C_INK);

    frame.drawPixel(x + blockWidth / 2 + 2, y + blockHeight - 8, C_GOLD);
  } else {
    for (uint8_t column = 0; column < 3; ++column) {
      const int16_t windowX = x + 5 + column * ((blockWidth - 10) / 3);

      const bool lit = ((seed >> (column * 3)) & 3U) != 0;

      frame.fillRect(windowX, y + 6, 6, 7, C_INK);

      frame.fillRect(windowX + 1, y + 7, 4, 5, lit ? C_GOLD : C_GLASS_DARK);
    }
  }

  if (style % 4 == 0 && blockHeight >= 34) {
    frame.drawFastHLine(x + 4, y + 5, blockWidth - 9, blend565(body, dark, 75));

    frame.drawFastHLine(x + 4, y + blockHeight - 14, blockWidth - 9,
                        blend565(body, dark, 75));
  } else if (style % 4 == 3 && blockHeight >= 34) {
    frame.drawFastVLine(x + blockWidth / 2, y + 4, blockHeight - 9,
  ...
  if (quality == LandingQuality::ROUGH && !hanging) {
    ...
    frame.drawPixel(x + blockWidth - 3, y + 1, C_WHITE);
  }

  if (hanging) {
    frame.fillRect(x + 7, y - 3, blockWidth - 14, 3, C_GOLD);

    frame.drawFastVLine(x + 7, y - 8, 6, C_INK);

    frame.drawFastVLine(x + blockWidth - 8, y - 8, 6, C_INK);
  }
}
...
static float towerSwayOffset(uint8_t index, uint8_t count, uint32_t now) {
  ...
  // The shared dynamic angle anchors the base while making high floors move
  // farther. The small secondary flex keeps the structure from looking rigid.
...
static void drawTower(uint32_t now) {
  ...
  const int16_t foundationWidth = (int16_t)roundf(FOUNDATION_WIDTH);

  frame.fillRect(foundationX - 4, baseY, foundationWidth + 8, 10, C_STONE_DARK);

  frame.fillRect(foundationX, baseY - 4, foundationWidth, 5, C_STONE);

  frame.drawFastHLine(foundationX, baseY - 4, foundationWidth, C_WHITE);

  for (uint8_t index = 0; index < city.floorCount; ++index) {
    const Floor &floor = city.floors[index];

    if (floor.detached) {
      drawFloorBlock(floor.detachedX, floor.detachedY, floor.width, floor.style,
                     floor.seed, false, floor.quality);
...
static void drawActiveBlock() {
  ...
  drawFloorBlock(city.active.centreX, city.active.topY, FLOOR_WIDTH,
                 ...
                 city.active.mode == BlockMode::HANGING, LandingQuality::GOOD);
}
...
static void drawMiniTower(uint32_t now) {
  ...
  for (uint8_t index = 0; index < 4; ++index) {
    ...
    drawFloorBlock(86 + sway, baseY - (index + 1) * 26, 34, index,
                   ...
                   index == 3 ? LandingQuality::PERFECT : LandingQuality::GOOD,
                   26.0f);
  }
  ...
  drawFloorBlock(hookX, 89, 34, 0, 0x9911U, true, LandingQuality::GOOD, 26.0f);
}
...
static void drawHowTo(uint32_t now) {
  ...
  if (city.howToPage == 0) {
    ...
    drawFloorBlock(hookX, 101, 44, 1, 0x1234U, true, LandingQuality::GOOD);

    drawFloorBlock(86, 184, 44, 0, 0x5678U, false, LandingQuality::GOOD);
    ...
    drawFloorBlock(86, 58, 44, 3, 0x1111U, false, LandingQuality::PERFECT);

    drawFloorBlock(86, 102, 44, 2, 0x2222U, false, LandingQuality::PERFECT);

    drawFloorBlock(86, 146, 44, 1, 0x3333U, false, LandingQuality::PERFECT);
...
static void drawResult(uint32_t now) {
  ...
  frame.drawRoundRect(10, 28, 152, 278, 12, city.completed ? C_GREEN : C_RED);
